...
import type { IWriteTrackMetadataBatchItem } from '@/app/shared/api/types'
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  if (entries.length > MAX_AUDIO_FILES_PER_BATCH) {
    return NextResponse.json(
      {
        success: false,
        error: `Too many files. Maximum: ${MAX_AUDIO_FILES_PER_BATCH}`,
      },
      { status: 400 }
    )
  }
  ...
  if (totalSize > MAX_AUDIO_BATCH_TOTAL_SIZE) {
    return NextResponse.json(
      {
        success: false,
  ...
  const tracks = parseTrackMetadataBatchItems(formData.get('tracks'))
  ...
  const tracksByIndex = new Map<number, IWriteTrackMetadataBatchItem>()
  ...
  console.log('[batch-write] cover payload', {
    coverMode,
    hasCoverFile: coverCandidate instanceof File,
    coverFileName: coverCandidate instanceof File ? coverCandidate.name : null,
    coverFileType: coverCandidate instanceof File ? coverCandidate.type : null,
    coverFileSize: coverCandidate instanceof File ? coverCandidate.size : null,
    coverDescription: typeof coverDescription === 'string' ? coverDescription : null,
  })
  ...
  try {
    ...
    for (const [index, file] of files.entries()) {
      ...
      if (index === 0) {
        console.log('[batch-write] first file cover input', {
          fileName: file.name,
